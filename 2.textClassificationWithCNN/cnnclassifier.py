...
Original file is located at
    https://colab.research.google.com/drive/1Tz_zH_26bj3IRUMOiYJ9T9nNvYJRSg2X
"""
...
def maybe_download(filename, expected_bytes):
    if not os.path.exists(os.path.join(filename)):
        filename, _ = urlretrieve(url + filename, os.path.join(filename))
    print(os.path.join(filename))
    statinfo = os.stat(os.path.join(filename))
    if statinfo.st_size == expected_bytes:
        print('Found and verified %s' % os.path.join(filename))
    else:
        print(statinfo.st_size)
        raise Exception(
            'Failed to verify ' + os.path.join(filename) + '. Can you get to it with a browser?')
    return filename


filename = maybe_download('train_1000.label', 60774)
test_filename = maybe_download('TREC_10.label', 23354)

filenames = ['train_1000.label', 'TREC_10.label']
num_files = len(filenames)
for i in range(len(filenames)):
    file_exists = os.path.isfile(os.path.join(filenames[i]))
    assert file_exists
...
for i in range(num_files):
    print('\nProcessing file %s' % os.path.join(filenames[i]))
    if i == 0:
        # Processing training data
        train_questions, train_labels = read_data(os.path.join(filenames[i]))
        # Making sure we got all the questions and corresponding labels
        assert len(train_questions) == len(train_labels)
    elif i == 1:
        # Processing testing data
        test_questions, test_labels = read_data(os.path.join(filenames[i]))
        # Making sure we got all the questions and corresponding labels.
